...
void MCP79410_Timer::getTime(char *buffer)
{
  char h = _getRtcData(2, 6);
  char m = _getRtcData(1, 7);
  char s = _getRtcData(0, 7);
  sprintf(buffer, "%d%d:%d%d:%d%d", h >> 4, h & 0x0f, m >> 4, m & 0x0f, s >> 4, s & 0x0f);
}
